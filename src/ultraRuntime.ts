...
const allText = (value: number) => new Intl.NumberFormat("en-US", { maximumFractionDigits: 2 }).format(value);
const normalizeZip = (value: string) => value.replace(/\D/g, "").slice(0, 4);
const isAlbanian = () => document.body.textContent?.includes("Rivendos") ?? false;
const inputValues = (placeholder: string) => Array.from(document.querySelectorAll<HTMLInputElement>(`input[placeholder="${placeholder}"]`)).map((input) => numberValue(input.value));
const totalWeight = () => round2(inputValues("kg").reduce((sum, value) => sum + value, 0));
const codEnabled = () => Array.from(document.querySelectorAll<HTMLInputElement>('input[type="checkbox"]')).some((input) => input.checked);
const additionalFee = (weight: number) => round2(Math.max(0, weight - INCLUDED_KG) * EXTRA_KG_ALL);
...
function ensureKosovoResult(results: UltraRuntimeResult[]) {
  if (direction !== "outbound" || country !== "XK" || results.some((result) => result.name === "Ultra")) return;
...
function applyDomestic(result: UltraRuntimeResult, weight: number, albanian: boolean) {
  ...
  if (!found) {
    ...
    result.details = [
      ...
      albanian
        ...
        : "The area and price are determined automatically from the postal code.",
    ];
    return;
  }

  const area = AREAS[found.area];
  const extra = additionalFee(weight);
  const total = round2(area.price + extra);
  const details = albanian
    ? [
        `${MARKER}: çmimet përfshijnë TVSH`,
        `Kodi postar: ${found.code}`,
        `Filiali / rrethi: ${found.districtSq}`,
        `Zona: ${area.sq}`,
        `Koha normale e dorëzimit: ${area.etaSq}`,
        `Pesha reale totale e dërgesës: ${weight.toFixed(2)} kg`,
        `Tarifa 0–2 kg: ${area.price} ALL`,
    ...
    : [
        `${MARKER}: prices include VAT`,
        `Postal code: ${found.code}`,
        `Postal branch / district: ${found.districtEn}`,
        `Area: ${area.en}`,
        `Normal delivery time: ${area.etaEn}`,
        `Total actual shipment weight: ${weight.toFixed(2)} kg`,
        `0–2 kg tariff: ${area.price} ALL`,
  ...
  if (extra > 0) {
    details.splice(
      ...
      0,
      albanian
        ? `Mbi 2 kg: ${EXTRA_KG_ALL} ALL/kg (+${allText(extra)} ALL)`
        : `Above 2 kg: ${EXTRA_KG_ALL} ALL/kg (+${allText(extra)} ALL)`,
    );
  }
...
function applyKosovo(result: UltraRuntimeResult, weight: number, albanian: boolean) {
  const extra = additionalFee(weight);
  result.serviceType = "MBE Express";
  ...
  result.status = "ok";
  result.warning = undefined;
  result.details = albanian
    ? [
        `${MARKER}: çmimet përfshijnë TVSH`,
        "Destinacioni: Kosovë",
        "Koha normale e dorëzimit: 4 ditë pune",
        `Pesha reale totale e dërgesës: ${weight.toFixed(2)} kg`,
        `Tarifa 0–2 kg: ${KOSOVO_BASE_EUR} EUR`,
    ...
    : [
        ...
        `Total actual shipment weight: ${weight.toFixed(2)} kg`,
        `0–2 kg tariff: €${KOSOVO_BASE_EUR.toFixed(2)}`,
        ...(extra > 0 ? [`Above 2 kg: ${EXTRA_KG_ALL} ALL/kg (+${allText(extra)} ALL)`] : []),
        "Charged once for the complete shipment, including multi-package shipments.",
      ];
}

function applyUltra(results: UltraRuntimeResult[]) {
  ensureKosovoResult(results);
  const result = results.find((item) => item.name === "Ultra");
  if (!result || result.details.some((detail) => detail.startsWith(MARKER))) return;

  result.serviceType = "MBE Express";
  if (direction === "outbound" && country === "AL") applyDomestic(result, totalWeight(), isAlbanian());
  else if (direction === "outbound" && country === "XK") applyKosovo(result, totalWeight(), isAlbanian());
}
...
function updateZipUi() {
  ...
  postalCode = normalizeZip(input.value);
  ...
  const labelText = albanian ? "Kodi postar i destinacionit" : "Destination postal code";
  const helpText = albanian
...
document.addEventListener("input", (event) => {
  ...
  const next = country === "AL"
    ? normalizeZip(input.value)
...
Array.prototype.filter = function <T>(
  ...
  if (this.some(isResult)) applyUltra(this as unknown as UltraRuntimeResult[]);
